...
old_folder_name = f"{root_dir}/data/Task01_BrainTumour"
new_folder_name = f"{root_dir}/data/BrainTumourData"
...
def extract_dataset(output):
    with tarfile.open(output, "r") as tar:
        tar.extractall(f"{root_dir}/data")
...
def main():
    ...
    parser.add_argument(
        "--manual", action="store_true", help="If set, skips the dataset download"
    )
